...
def test_success_return_status_code_201_create(client, employee, competency) -> None:
    ...
    entity = EmployeeCompetency(id=1,
                                       employee_id=1,
                                       competency_id=1,
...
def test_success_return_status_code_204_delete(client, employee, department, employee_department) -> None:
    ...
    response = test_client.delete(
        f"api/v1/employee_competency/id/{department[0].id}"
    )
...
def test_success_return_status_code_201_bulk(client) -> None:
    ...
    entity = [EmployeeCompetency(id=1,
                        ...
                        EmployeeCompetency(id=3,
                                       ...
                                       current_level=RequiredLevelEnum.advanced.value)]
    response = test_client.post(
        "api/v1/employee_competency/bulk",
        json=[e.model_dump() for e in entity]
    )
    assert response.status_code == 201

def test_success_return_status_code_200_get_all_by_employee(client,  employee, department, competency,
                                             employee_department, employee_competency) -> None:
    test_client, _, _ = client


    response = test_client.get(
        f"api/v1/employee_competency/employee/{employee[0].id}"
    )
    assert response.status_code == 200

def test_success_return_status_code_200_group_competency_level_by_employee_ids(client,  employee, department, competency,
                                             employee_department, employee_competency) -> None:
    test_client, _, _ = client


    response = test_client.get(
        f"api/v1/employee_competency/group/department/{department[0].id}"
    )
    assert response.status_code == 200

def test_success_return_status_code_200_get_all_by_department(client,  employee, department, competency,
                                             employee_department, employee_competency) -> None:
    test_client, _, _ = client


    response = test_client.get(
        f"api/v1/employee_competency/department/{department[0].id}"
    )
    assert response.status_code == 200

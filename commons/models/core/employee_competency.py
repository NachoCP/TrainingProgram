...
class EmployeeCompetency(BaseModel):
    ...
    current_level: RequiredLevelEnum = Field(..., description="The current level of the competency for the employee", example="Intermediate")
    expiration_date: Optional[date] = Field(description="Date when the role or assignment expires", example="2023-12-31", default=None)
    effective_date: Optional[date] = Field(description="Date when the role became effective", example="2021-06-01", default=None)

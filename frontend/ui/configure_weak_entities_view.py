...
def weak_entities_view():
    ...
    st.markdown("""Format table will all the feedback, relationship between employee-competencies and
    ...
    df_competency_levels = pd.DataFrame([competency_level.model_dump() for competency_level in st.session_state.competency_levels])
    df_employee_competencies = pd.DataFrame([employee_competency.model_dump() for employee_competency in st.session_state.employee_competencies])
    df_employee_department = pd.DataFrame([employee_department.model_dump() for employee_department in st.session_state.employee_departments])

    tab1, tab2, tab3, tab4 = st.tabs(["Feedback Table",
    ...
    df_final_feedback = (
        df_feedbacks
        .merge(df_employees, left_on="employee_id", right_on="id", how="left", suffixes=("", "_employee")).drop("id_employee", axis=1)
        .merge(df_employees, left_on="feedback_by", right_on="id", how="left", suffixes=("", "_feedback_by")).drop("id_feedback_by", axis=1)
        .rename(columns={"name": "employee_name", "name_feedback_by": "employee_feedback_by_name"})
    )[["id", "employee_id", "employee_name", "feedback_by", "employee_feedback_by_name", "comments", "score", "effective_date"]]
    #print(df_final_feedback)

    df_final_employee_competencies = (
        df_employee_competencies
        .merge(df_employees, left_on="employee_id", right_on="id", how="left", suffixes=("", "_employee")).drop("id_employee", axis=1)
        .merge(df_competencies, left_on="competency_id", right_on="id", how="left", suffixes=("", "_competency")).drop("id_competency", axis=1)
    ...
    df_final_employee_department = (
        df_employee_department
        .merge(df_employees, left_on="employee_id", right_on="id", how="left", suffixes=("", "_employee")).drop("id_employee", axis=1)
        .merge(df_departments, left_on="department_id", right_on="id", how="left", suffixes=("", "_department")).drop("id_department", axis=1)

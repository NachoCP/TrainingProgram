...
def main():
    ...
    if st.session_state.page == ViewEnum.employee_view.value:
        ...
        expected_competency_levels_department = CompetencyLevelService().get_all_by_department(st.session_state.department.id)
        real_competency_levels_department = EmployeeCompetencyService().get_all_group_department(st.session_state.department.id)

        df_expected_competency_levels_department = pd.DataFrame([d.model_dump() for d in expected_competency_levels_department])
        df_real_competency_levels_department = pd.DataFrame([d.model_dump() for d in real_competency_levels_department])

        df_merged_levels = df_expected_competency_levels_department.merge(df_real_competency_levels_department,
                                                                on=["name", "required_level"], how="outer",
        ...
        df_merged_levels = df_merged_levels.rename(columns={"name": "Competency", "required_level": "Level", "diff_num_workers": "Needed"})
        ...
        st.sidebar.dataframe(df_merged_levels[["Competency", "Level", "Needed"]], hide_index=True, use_container_width=True)
    view()

...
def create(data: Feedback, db: Session = Depends(get_db_connection)):  # noqa: B008
...
@router.get("", status_code=status.HTTP_202_ACCEPTED, response_model=List[Feedback])
...
def update(id: int, data: Feedback, db: Session = Depends(get_db_connection)):  # noqa: B008
    _service = FeedbackService(db)
    return _service.update(id, data)


@router.post("/bulk", status_code=status.HTTP_201_CREATED, response_model=List[Feedback])
def bulk(data: List[Feedback], db: Session = Depends(get_db_connection)):  # noqa: B008
    _service = FeedbackService(db)

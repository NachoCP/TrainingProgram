...
router = APIRouter(prefix="/employee_competency", tags=["employee_competency"])


@router.post("", status_code=status.HTTP_201_CREATED, response_model=EmployeeCompetency)
def create(data: EmployeeCompetency, db: Session = Depends(get_db_connection)):  # noqa: B008
...
def update(id: int, data: EmployeeCompetency, db: Session = Depends(get_db_connection)):  # noqa: B008
    _service = EmployeeCompetencyService(db)
...
def bulk(data: List[EmployeeCompetencyWithoutDates], db: Session = Depends(get_db_connection)):  # noqa: B008
    _service = EmployeeCompetencyService(db)
...
def get_all_by_employee(id: int, db: Session = Depends(get_db_connection)):  # noqa: B008
    _service = EmployeeCompetencyService(db)
    return _service.get_all_by_employee(id)


@router.get(
    "/group/department/{department_id}", status_code=status.HTTP_200_OK, response_model=List[CompetencyLevelOutput]
)
def group_competency_level_by_employee_ids(department_id: int, db: Session = Depends(get_db_connection)):  # noqa: B008
    _service = EmployeeCompetencyService(db)
    return _service.group_competency_level_by_employee_ids(department_id)


@router.get(
    "/department/{department_id}", status_code=status.HTTP_200_OK, response_model=List[CompetencyLevelDepartmentOutput]
)

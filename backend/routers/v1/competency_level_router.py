...
router = APIRouter(prefix="/competency_level", tags=["competency_level"])


@router.post("", status_code=status.HTTP_201_CREATED, response_model=CompetencyLevel)
def create(data: CompetencyLevel, db: Session = Depends(get_db_connection)):  # noqa: B008
...
def update(id: int, data: CompetencyLevel, db: Session = Depends(get_db_connection)):  # noqa: B008
    _service = CompetencyLevelService(db)
    return _service.update(id, data)


@router.post("/bulk", status_code=status.HTTP_201_CREATED, response_model=List[CompetencyLevel])
def bulk(data: List[CompetencyLevel], db: Session = Depends(get_db_connection)):  # noqa: B008
    _service = CompetencyLevelService(db)

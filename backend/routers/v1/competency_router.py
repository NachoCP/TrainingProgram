...
def create(data: Competency, db: Session = Depends(get_db_connection)):  # noqa: B008
    _service = CompetencyService(db)
    return _service.create(data)


@router.get("/id/{id}", status_code=status.HTTP_200_OK, response_model=Competency)
def get(id: int, db: Session = Depends(get_db_connection)):  # noqa: B008
    _service = CompetencyService(db)
    return _service.get(id)


@router.get("", status_code=status.HTTP_200_OK, response_model=List[Competency])
...
@router.put("/id/{id}", status_code=status.HTTP_202_ACCEPTED, response_model=Competency)
def update(id: int, data: Competency, db: Session = Depends(get_db_connection)):  # noqa: B008
    _service = CompetencyService(db)
    return _service.update(id, data)


@router.post("/bulk", status_code=status.HTTP_201_CREATED, response_model=List[Competency])
def bulk(data: List[Competency], db: Session = Depends(get_db_connection)):  # noqa: B008
    _service = CompetencyService(db)
